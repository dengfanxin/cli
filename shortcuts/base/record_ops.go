...
func executeRecordDelete(runtime *common.RuntimeContext) error {
	...
	recordID := runtime.Str("record-id")
	stop, err := handleDeleteApproval(runtime, deleteApprovalSpec{
		Action: "base.record.delete", BaseToken: baseToken, ResourceType: "record", ResourceID: tableID + ":" + recordID,
	})
	if err != nil || stop {
		return err
	}
	_, err = baseV3Delete(runtime, baseV3Path("bases", baseToken, "tables", tableID, "records", recordID))
	if err != nil {
		return err
	}
	runtime.Out(map[string]interface{}{"deleted": true, "record_id": recordID}, nil)
	return nil
}
